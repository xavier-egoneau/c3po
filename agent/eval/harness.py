...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--agent", action="store_true", help="avec --model : égaliseur par sélection de contexte (au lieu du dump-all brut)")
    ...
    if args.model:
        from agent.eval.solvers import engine_chat, make_context_solver, make_oneshot_solver

        label = args.label or ("small_agent" if args.agent else "small")
        chat = engine_chat(args.model)
        try:
            solver = make_context_solver(chat) if args.agent else make_oneshot_solver(chat)
